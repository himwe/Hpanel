...
sed -i "s@^DocumentRoot.*@DocumentRoot \"$home_default\"@" $apache_install_dir/conf/httpd.conf
sed -i "s@^<Directory \"$apache_install_dir/htdocs\">@<Directory \"$home_default\">@" $apache_install_dir/conf/httpd.conf
