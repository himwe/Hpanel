...
#-*- coding: utf-8 -*-
#
# Copyright (c) 2012, Hpanel development team
# All rights reserved.
#
# Hpanel is distributed under the terms of the (new) BSD License.
# The full license can be found in 'LICENSE.txt'.

""" Install script for Hpanel """
...
class Install(object):
    ...
    #def install_hpanel(self):
        #localpkg_found = False
        #if os.path.exists(os.path.join(os.path.dirname(__file__), 'hpanel.zip')):
            # local install package found
            #localpkg_found = True
        #elif:
            # or else install online
            #print '* Downloading install package . . . . . .'
            #self._run('wget -nv -c http://dl.himwe.com/source/panel/hpanel.zip')
        #elif:
            # or else install online
          #  print '* Downloading install package . . . . . .'
         #   self._run('wget -nv -c https://github.com/himwe/Hpanel/archive/master.zip')
            
        # uncompress and install it
        #self._run('unzip hpanel.zip')
        #if not localpkg_found: os.remove('hpanel.zip')
        
        # stop service
        print
        if os.path.exists('/etc/init.d/hpanel'):
            self._run('/etc/init.d/hpanel stop')

        # backup data and remove old code
        if os.path.exists('%s/data/' % self.installpath):
            self._run('cp -r %s/data/* Panel/data/' % self.installpath, True)
        self._run('rm -rf %s' % self.installpath)
        
        # install new code
        self._run('mv Panel %s' % self.installpath)
        self._run('chmod +x %s/config.py %s/server.py' % (self.installpath, self.installpath))
        
        # install service
        initscript = '%s/tools/init.d/%s/hpanel' % (self.installpath, self.distname)
        self._run('cp %s /etc/init.d/hpanel' % initscript)
        self._run('chmod +x /etc/init.d/hpanel')
        
        # start service
        if self.distname in ('centos', 'redhat'):
            self._run('chkconfig hpanel on')
            self._run('service hpanel start')
        elif self.distname == 'ubuntu':
            pass
        elif self.distname == 'debian':
            pass
    ...
    def install(self):
        ...
        # get the latest Hpanel version
        print '* Installing latest Hpanel'
        self.install_hpanel()
        ...
        print 
        print '* The URL of your Hpanel is:',
